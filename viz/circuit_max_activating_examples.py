...
def get_circuit_activations(
    ...
    """Compute per-token, per-circuit activations.

    Args:
        circuits: Circuit tensor of shape (C, L, E) or (C, L*E)
        token_topk_mask: Optional pre-loaded token mask of shape (B, L, E)
        device: Device to use for computation
        minibatch_size: Optional batch size for processing

    Returns:
        activations: Tensor of shape (B, C) with activation scores
        token_topk_mask: The token mask used for computation
    """
    # Load activations if not provided
    if token_topk_mask is None:
        token_topk_mask, _, _, _ = load_activations_indices_tokens_and_topk(
            device=device
        )

    # Ensure circuits is in the right shape
    circuits = _load_circuits_tensor(
        circuits if isinstance(circuits, str) else "",
        device=device,
