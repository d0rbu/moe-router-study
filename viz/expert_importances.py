...
@dataclass
class Component:
    """Component in the model with its display name."""
...
# Writers (blue)
WRITER_COMPONENTS = {
    Component("mlp.down_proj", "Down (writer)"),
    Component("attn.o_proj", "Attn O (writer)"),
}

# Readers (red)
READER_COMPONENTS = {
    Component("attn.q_proj", "Attn Q (reader)"),
    Component("attn.k_proj", "Attn K (reader)"),
    Component("mlp.up_proj", "Up (reader)"),
    Component("mlp.gate_proj", "Gate (reader)"),
}

# All components for iteration
ALL_COMPONENTS = WRITER_COMPONENTS | READER_COMPONENTS

# Expert-owned weights
...
def load_importance_data(path: str) -> ImportanceData:
    ...
    for comp in ALL_COMPONENTS:
        component_max_l2[comp.key] = 0.0

    # aggregate per layer/component
    for e in entries:
        layer = int(e["layer_idx"])
        comp = str(e["component"])
        expert_idx = int(e["expert_idx"])
        l2 = float(e["l2"])
        num_experts = int(e["num_experts"])

        if layer not in layer_component_l2:
            layer_component_l2[layer] = {comp.key: [] for comp in ALL_COMPONENTS}
...
def expert_importances() -> None:
    ...
    col_labels = (
        [comp.display_name for comp in WRITER_COMPONENTS]
        + ["Residual"]
        + [comp.display_name for comp in READER_COMPONENTS]
    )
    col_components = (
        [comp.key for comp in WRITER_COMPONENTS]
        + [None]
        + [comp.key for comp in READER_COMPONENTS]
    )
    ...
    def refresh() -> None:
        ...
        for row_idx, layer in enumerate(data.layers_sorted):
            ...
            for col_idx, comp in enumerate(col_components):
                ...
                # Find role
                role = (
                    "writer" if comp in {c.key for c in WRITER_COMPONENTS} else "reader"
                )
