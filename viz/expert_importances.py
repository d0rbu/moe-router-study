...
def expert_importances(
    ...
    layers = sorted(list(set(e["layer_idx"] for e in entries)))
    ...
    for entry in entries:
        ...
        importance_data[(layer_idx, component, expert_idx)] = {
            "role": role,
            "l2": l2
        }
    ...
    vmin = 0
    vmax = np.percentile(all_l2_values, normalize_percentile)
    ...
    # Set axis limits
    max_width = max(num_reader_components, num_writer_components) * (expert_width * num_experts + component_spacing)
    ...
    def update(val=None):
        ...
        # Update title
        ax.set_title(f"Expert Importances - Layer {new_layer_idx}, Expert {new_expert_idx}")
    ...
    ax.set_title(f"Expert Importances - Layer {layers[current_layer_idx]}, Expert {current_expert_idx}")
