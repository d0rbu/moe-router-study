...
class TestExpandBatch:
    ...
    def test_multidimensional_tensor_conversion(self):
        ...
        batch = {
            'matrix': th.tensor([[1, 2], [3, 4]]),
            'vector': th.tensor([10, 20])
        }
    ...
    def test_inconsistent_lengths_error(self):
        ...
        batch = {
            'short': th.tensor([1, 2]),
            'long': th.tensor([1, 2, 3, 4])
        }
        
        with pytest.raises(AssertionError, match="All values must have the same length"):
            expand_batch(batch)
    ...
    def test_float_tensor_precision(self):
        ...
        batch = {
            'precise_values': th.tensor([1.23456789, 9.87654321])
        }
