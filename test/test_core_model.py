...
class TestCheckpoint:
    ...
    def test_checkpoint_creation(self):
        ...
        checkpoint = Checkpoint(step=1000, num_tokens=5000, model_config=model_config)
    ...
    def test_checkpoint_str_with_revision(self):
        ...
        model_config = ModelConfig(hf_name="test/model")
        checkpoint = Checkpoint(
            step=1000, num_tokens=5000, model_config=model_config, revision="main"
        )
    ...
    def test_checkpoint_str_with_format(self):
        ...
        model_config = ModelConfig(
            hf_name="test/model", revision_format="step{}-tokens{}B"
        )
        checkpoint = Checkpoint(step=1000, num_tokens=5000, model_config=model_config)
    ...
    def test_checkpoint_str_none_values(self):
        ...
        checkpoint = Checkpoint(step=None, num_tokens=None, model_config=model_config)
...
class TestModelConfig:
    ...
    def test_latest_checkpoint_property(self):
        ...
        config = ModelConfig(
            hf_name="test/model", total_steps=1000, total_tokens=5000, eager_fetch=False
        )
    ...
    def test_get_checkpoint_by_step(self):
        ...
        config = ModelConfig(
            hf_name="test/model", revision_format="step{}-tokens{}B", eager_fetch=False
        )
        config.checkpoints = [
            Checkpoint(1000, 5000, config),
            Checkpoint(2000, 10000, config),
...
class TestModelsConstant:
    ...
    def test_models_content(self):
        ...
        expected_models = ["olmoe", "phimoe", "q3", "gpt", "olmoe-i"]
...
class TestModelIntegration:
    ...
    def test_full_workflow(self, mock_list_refs):
        ...
        # Mock HuggingFace response
        mock_refs = create_mock_hf_refs(
            ["step1000-tokens5000B", "step2000-tokens10000B", "main"]
        )
        mock_list_refs.return_value = mock_refs
