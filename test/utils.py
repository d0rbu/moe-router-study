...
def create_synthetic_router_logits(
    batch_size: int = 2, seq_len: int = 4, num_experts: int = 8, seed: int = 42
) -> th.Tensor:
...
def create_synthetic_activations(
    batch_size: int = 2, seq_len: int = 4, hidden_dim: int = 64, seed: int = 42
) -> th.Tensor:
...
def skip_if_no_network():
    ...
    return pytest.mark.skipif(not check_network(), reason="Network not available")
...
def parametrize_dtypes():
    ...
    return pytest.mark.parametrize("dtype", [th.float32, th.float16, th.bfloat16])
