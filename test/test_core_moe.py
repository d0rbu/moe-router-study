...
class TestConvertRouterLogitsToPaths:
    ...
    def test_basic_functionality(self):
        ...
        router_logits = th.tensor([
            [[1.0, 2.0, 0.5, 3.0],  # Expert 3 and 1 should be top-2
             [0.1, 4.0, 2.0, 1.0]]  # Expert 1 and 2 should be top-2
        ])  # Shape: (1, 2, 4)
        ...
        for b in range(batch_size):
            for s in range(seq_len):
                ...
                assert selected_experts == top_k, f"Expected {top_k} experts, got {selected_experts}"
    ...
    def test_gradient_flow(self):
        ...
        output = result.sum()
        assert output.item() == top_k  # Should equal top_k since we select top_k experts per position
    ...
    @pytest.mark.parametrize("batch_size,seq_len,num_experts,top_k", [
        (1, 1, 4, 2),
        (2, 3, 8, 3),
        (4, 2, 6, 1),
        (1, 5, 10, 5),
