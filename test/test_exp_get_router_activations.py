...
class TestSaveRouterLogits:
    ...
    def test_save_router_logits(self, temp_dir, monkeypatch):
        ...
        tokenized_batch_collection = [["token1", "token2"], ["token3", "token4", "token5"]]
        top_k = 2
        file_idx = 0
        
        # Set output directory to temp_dir
        monkeypatch.setattr("exp.get_router_activations.ROUTER_LOGITS_DIR", str(temp_dir))
        
        # Call the function
        save_router_logits(router_logit_collection, tokenized_batch_collection, top_k, file_idx)
        ...
        assert th.equal(saved_data["router_logits"], th.cat(router_logit_collection, dim=0))
...
class TestProcessBatch:
    ...
    def test_process_batch(self):
        ...
        mock_model.tokenizer.return_value = MagicMock(attention_mask=th.tensor([[1, 1, 0], [1, 0, 0]]))
        mock_model.tokenizer.tokenize.side_effect = lambda text: [f"{text}_token_{i}" for i in range(3)]
        ...
        mock_model.routers_output = {
            0: MagicMock(cpu=lambda: th.randn(6, 4)),  # 6 tokens (flattened), 4 experts
            1: MagicMock(cpu=lambda: th.randn(6, 4)),
        }
        # Make the cpu() method return a tensor with a save method
        for layer in [0, 1]:
            mock_model.routers_output[layer].cpu().save = lambda: mock_model.routers_output[layer].cpu()
        
        # Call the function
        batch = ["text1", "text2"]
        router_layers = [0, 1]
        router_logits, tokenized_batch = process_batch(batch, mock_model, router_layers)
        
        # Check that the tokenizer was called correctly
        mock_model.tokenizer.assert_called_once_with(batch, padding=True, return_tensors="pt")
        ...
        assert router_logits.shape[0] == 3  # 3 tokens with padding mask True
        assert router_logits.shape[1] == 2  # 2 layers
        assert router_logits.shape[2] == 4  # 4 experts
...
class TestGetRouterActivations:
    ...
    def test_get_router_activations_invalid_model(self):
        ...
        with patch("exp.get_router_activations.MODELS", {}):
    ...
    def test_get_router_activations_invalid_dataset(self):
        ...
        with patch("exp.get_router_activations.MODELS", {"test_model": MagicMock()}), \
             patch("exp.get_router_activations.DATASETS", {}):
            with pytest.raises(ValueError, match="Dataset .* not found"):
                get_router_activations(model_name="test_model", dataset="nonexistent_dataset")
    ...
    def test_get_router_activations_basic(self, temp_dir, monkeypatch):
        ...
        def mock_process_batch(batch, model, router_layers):
            ...
            return th.randn(len(batch), len(router_layers), 4), [f"{text}_tokenized" for text in batch]
        
        # Set up patches
        monkeypatch.setattr("exp.get_router_activations.OUTPUT_DIR", str(temp_dir))
        monkeypatch.setattr("exp.get_router_activations.ROUTER_LOGITS_DIR", str(temp_dir))
        
        with patch("exp.get_router_activations.MODELS", mock_models), \
             patch("exp.get_router_activations.DATASETS", mock_datasets), \
             patch("exp.get_router_activations.CUSTOM_DEVICES", {"cpu": lambda: "cpu"}), \
             patch("exp.get_router_activations.StandardizedTransformer", return_value=mock_transformer), \
             patch("exp.get_router_activations.process_batch", side_effect=mock_process_batch), \
    ...
    def test_get_router_activations_with_custom_device(self):
        ...
        with patch("exp.get_router_activations.MODELS", mock_models), \
             ...
             patch("exp.get_router_activations.StandardizedTransformer") as mock_transformer_cls, \
            ...
            assert kwargs["device_map"] == {"model.layers.0": "cuda:0", "model.layers.1": "cuda:1"}
