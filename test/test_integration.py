...
class TestActivationToLossPipeline:
    ...
    def test_activation_to_circuit_loss_flow(self, temp_dir):
        ...
        num_circuits = 8
        circuits_logits = create_sample_circuit_logits(num_circuits, num_layers, num_experts)
...
class TestLossToOptimizationPipeline:
    ...
    def test_loss_to_wandb_logging_flow(self):
        ...
        router_data = create_sample_router_data(batch_size, num_layers, num_experts, topk)
...
class TestVisualizationPipeline:
    ...
    def test_weights_to_router_spaces_flow(self, temp_dir):
        ...
        # Mock the visualization pipeline
        with patch('exp.get_weights.WEIGHT_DIR', str(temp_dir)), \
...
class TestEndToEndDataFlow:
    ...
    def test_complete_pipeline_simulation(self, temp_dir):
        ...
        assert_tensor_shape_and_type(activated_experts, (batch_size, num_layers, num_experts), th.bool)
        ...
        for item in expanded:
            assert all(key in item for key in ['total_loss', 'faithfulness_loss', 'complexity', 'step'])
        
        # Step 6: Visualization
        with patch('torch_pca.PCA') as mock_pca_class, \
