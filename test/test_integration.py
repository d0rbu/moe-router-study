...
class TestEndToEndDataFlow:
    ...
    def test_complete_pipeline_simulation(self, temp_dir):
        ...
        with patch("datasets.load_dataset") as mock_load_dataset:
            ...
            # override toy to return mocked text to fully isolate from local toy data
            original = DATASETS["toy"]
            DATASETS["toy"] = lambda: mock_dataset["text"]  # type: ignore[assignment]
            try:
                dataset_func = DATASETS["toy"]
                text_data = list(dataset_func())
            finally:
                DATASETS["toy"] = original
...
class TestErrorPropagation:
    ...
    def test_activation_loading_error_propagation(self, temp_dir):
        ...
        with patch("exp.activations.ROUTER_LOGITS_DIR", str(temp_dir)):
            ...
            with pytest.raises(Exception):  # noqa: B017 - propagate broad error from mock
    ...
    def test_circuit_loss_error_propagation(self):
        ...
        with pytest.raises(AssertionError):
            # pass top_k to match function signature
            circuit_loss(data, circuits_logits, top_k=2)
