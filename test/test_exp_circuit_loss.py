...
class TestMaxIouAndIndex:
    ...
    def test_basic_iou_calculation(self, sample_bool_tensor_3d, sample_circuits_tensor):
        ...
        max_iou, max_iou_idx = max_iou_and_index(
    ...
    def test_input_validation(self):
        ...
        with pytest.raises(
            AssertionError, match="circuits must have the same number of layers"
        ):
            max_iou_and_index(
                th.zeros(2, 3, 4, dtype=th.bool), th.zeros(1, 5, 4, dtype=th.bool)
            )

        # Test mismatched expert dimensions
        with pytest.raises(
            AssertionError, match="circuits must have the same number of experts"
        ):
            max_iou_and_index(
                th.zeros(2, 3, 4, dtype=th.bool), th.zeros(1, 3, 6, dtype=th.bool)
            )

        # Test wrong circuits dtype
        with pytest.raises(AssertionError, match="circuits must be a boolean tensor"):
            max_iou_and_index(
                th.zeros(2, 3, 4, dtype=th.bool), th.zeros(1, 3, 4, dtype=th.float32)
            )
...
class TestMeanMaxIou:
    ...
    def test_mean_consistency_with_max_iou(
        self, sample_bool_tensor_3d, sample_circuits_tensor
    ):
...
class TestHardCircuitScore:
    ...
    def test_complexity_coefficient_effect(
        self, sample_bool_tensor_3d, sample_circuits_tensor
    ):
    ...
    def test_complexity_power_effect(
        self, sample_bool_tensor_3d, sample_circuits_tensor
    ):
    ...
    def test_zero_complexity_coefficient(
        self, sample_bool_tensor_3d, sample_circuits_tensor
    ):
...
class TestMinLogitLossAndIndex:
    ...
    def test_input_validation_logit_loss(self):
        ...
        with pytest.raises(
            AssertionError, match="circuits_logits must have the same number of layers"
        ):
            min_logit_loss_and_index(
                th.zeros(2, 3, 4, dtype=th.bool), th.zeros(1, 5, 4)
            )

        # Test wrong dtype
        with pytest.raises(
            AssertionError, match="circuits_logits must be a float32 tensor"
        ):
            min_logit_loss_and_index(
                th.zeros(2, 3, 4, dtype=th.bool), th.zeros(1, 3, 4, dtype=th.int32)
            )
...
class TestMinLogitLoss:
    ...
    def test_consistency_with_min_logit_loss_and_index(self, sample_bool_tensor_3d):
        ...
        loss_individual, _ = min_logit_loss_and_index(
            sample_bool_tensor_3d, circuits_logits
        )
...
class TestCircuitLoss:
    ...
    def test_loss_components_relationship(self, sample_bool_tensor_3d):
        ...
        expected_total = (
            ...
            + complexity_importance * complexity
        )
...
class TestCircuitLossIntegration:
    ...
    def test_loss_functions_consistency(self):
        ...
        circuits_logits = create_sample_circuit_logits(
            num_circuits=5, num_layers=3, num_experts=8
        )
        ...
        min_loss = min_logit_loss(activated_experts, circuits_logits)
