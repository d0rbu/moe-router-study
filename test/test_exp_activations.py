...
class TestActivationLoadingErrorHandling:
    ...
    def test_corrupted_file_handling(self, temp_dir):
        ...
        with (
            patch("exp.activations.ROUTER_LOGITS_DIR", str(temp_dir)),
            pytest.raises(RuntimeError),  # torch.load should raise on corrupted file
        ):
            load_activations_and_indices_and_topk()
    ...
    def test_invalid_tensor_shapes(self, temp_dir):
        ...
        with (
            ...
            pytest.raises(
                RuntimeError
            ),  # invalid tensor shapes should error in torch ops
        ):
            load_activations_and_indices_and_topk()
    ...
    def test_zero_topk_value(self, temp_dir):
        ...
        with (
            patch("exp.activations.ROUTER_LOGITS_DIR", str(temp_dir)),
            pytest.raises(RuntimeError),  # torch.topk with k=0 invalid
        ):
            load_activations_and_indices_and_topk()
    ...
    def test_topk_larger_than_experts(self, temp_dir):
        ...
        with (
            patch("exp.activations.ROUTER_LOGITS_DIR", str(temp_dir)),
            pytest.raises(RuntimeError),  # torch.topk with k>dim invalid
        ):
            load_activations_and_indices_and_topk()
