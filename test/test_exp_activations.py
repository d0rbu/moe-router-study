...
from test.test_utils import assert_tensor_shape_and_type, create_temporary_data_files
...
class TestLoadActivationsAndIndicesAndTopk:
    ...
    def test_basic_loading(self, temp_dir):
        ...
        assert_tensor_shape_and_type(activated_experts, (expected_batch_size, 3, 8), th.bool)
        assert_tensor_shape_and_type(activated_indices, (expected_batch_size, 3, 2))  # topk=2
    ...
    def test_single_file(self, temp_dir):
        ...
        data = {
            'topk': 1,
            'router_logits': th.randn(10, 4, 12)
        }
    ...
    def test_activation_logic(self, temp_dir):
        ...
        # Check first sample
        assert activated_experts[0, 0, 1] == True  # Expert 1 should be active
        assert activated_experts[0, 0, 2] == True  # Expert 2 should be active
        assert activated_experts[0, 0, 0] == False  # Expert 0 should not be active
        assert activated_experts[0, 0, 3] == False  # Expert 3 should not be active
        
        # Check second sample
        assert activated_experts[1, 0, 2] == True  # Expert 2 should be active
        assert activated_experts[1, 0, 3] == True  # Expert 3 should be active
        assert activated_experts[1, 0, 0] == False  # Expert 0 should not be active
        assert activated_experts[1, 0, 1] == False  # Expert 1 should not be active
    ...
    def test_device_handling(self, temp_dir):
        ...
        with patch('exp.activations.ROUTER_LOGITS_DIR', str(temp_dir)):
            ...
            activated_experts, _, _ = load_activations_and_indices_and_topk(device="cpu")
    ...
    def test_large_batch_concatenation(self, temp_dir):
        ...
        for i, batch_size in enumerate(batch_sizes):
            data = {
                'topk': 2,
                'router_logits': th.randn(batch_size, 3, 6)
            }
            th.save(data, temp_dir / f"{i}.pt")
        ...
        assert_tensor_shape_and_type(activated_experts, (total_batch_size, 3, 6), th.bool)
...
class TestActivationLoadingIntegration:
    ...
    def test_realistic_data_loading(self, temp_dir):
        ...
        assert_tensor_shape_and_type(activated_experts, (total_tokens, num_layers, num_experts), th.bool)
        assert_tensor_shape_and_type(activated_indices, (total_tokens, num_layers, topk))
    ...
    def test_memory_efficiency(self, temp_dir):
        ...
        for i in range(num_files):
            data = {
                'topk': 2,
                'router_logits': th.randn(100, 6, 16)
            }
            th.save(data, temp_dir / f"{i}.pt")
    ...
    def test_different_layer_expert_combinations(self, temp_dir):
        ...
        for num_layers, num_experts in test_cases:
            ...
            with patch('exp.activations.ROUTER_LOGITS_DIR', str(temp_dir)):
                activated_experts, _, _ = load_activations_and_indices_and_topk()
                assert_tensor_shape_and_type(activated_experts, (50, num_layers, num_experts), th.bool)
