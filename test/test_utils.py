...
def assert_tensor_shape_and_type(tensor: th.Tensor, expected_shape: tuple, expected_dtype: th.dtype = None) -> None:
...
def assert_tensor_close(actual: th.Tensor, expected: th.Tensor, rtol: float = 1e-5, atol: float = 1e-8) -> None:
...
class TensorGenerator:
    ...
    @staticmethod
    def boolean_tensor(shape: tuple, true_probability: float = 0.3, device: str = "cpu") -> th.Tensor:
        """Generate a boolean tensor with specified probability of True values."""
        return th.rand(shape, device=device) < true_probability
    
    @staticmethod
    def normalized_tensor(shape: tuple, dim: int = -1, device: str = "cpu") -> th.Tensor:
    ...
    @staticmethod
    def sparse_tensor(shape: tuple, sparsity: float = 0.1, device: str = "cpu") -> th.Tensor:
...
def validate_device_map(device_map: Dict[str, str], expected_devices: List[str]) -> None:
    ...
    for key, device in device_map.items():
        ...
        assert device in expected_devices or device == "cpu" or isinstance(device, int), \
    ...
    assert len(found_patterns) > 0, "Device map should contain expected model component keys"
...
def assert_valid_loss(loss: th.Tensor) -> None:
    ...
    assert th.all(loss >= 0.0), f"Loss values must be non-negative, got min: {th.min(loss)}"
