...
from exp.expert_importance import expert_importance, EXPERT_IMPORTANCE_DIR
...
class TestExpertImportance:
    ...
    def test_expert_importance_basic(self, temp_dir, monkeypatch):
        ...
        monkeypatch.setattr("exp.expert_importance.EXPERT_IMPORTANCE_DIR", str(temp_dir))
        
        with patch("exp.expert_importance.MODELS", mock_models), \
             patch("exp.expert_importance.StandardizedTransformer", return_value=mock_transformer):
        ...
        with patch("exp.expert_importance.MODELS", mock_models), \
            ...
            for entry in entries:
                if entry["expert_idx"] == 0 and entry["component"] == "mlp.up_proj":
                    ...
                    assert th.allclose(entry["importance_vector"], th.tensor([5.0, 0.0]))
                ...
                if entry["expert_idx"] == 1 and entry["component"] == "attn.q_proj":
                    ...
                    assert th.allclose(entry["importance_vector"], th.tensor([0.0, 2.0]))
    ...
    def test_expert_importance_with_revision(self, temp_dir, monkeypatch):
        ...
        with patch("exp.expert_importance.MODELS", mock_models), \
             patch("exp.expert_importance.StandardizedTransformer", return_value=mock_transformer) as mock_transformer_cls:
