...
class TestLossMonotonicityValidator:
    ...
    def test_validate_monotonic_increase_below_threshold(self):
        ...
        config = ValidationConfig(monotonicity_window_size=3, monotonicity_threshold=0.1)
        validator = LossMonotonicityValidator(config)
    ...
    def test_validate_monotonic_increase_above_threshold(self):
        ...
        config = ValidationConfig(monotonicity_window_size=3, monotonicity_threshold=0.01)
        validator = LossMonotonicityValidator(config)
...
class TestCentroidDistributionValidator:
    ...
    async def test_validate_empty_centroids(self):
        ...
        assert len(result["warnings"]) > 0
        assert any("have no assigned validation points" in w for w in result["warnings"])
    ...
    async def test_validate_imbalanced_distribution(self):
        ...
        assert len(result["warnings"]) > 0
        assert any("Highly imbalanced centroid assignments" in w for w in result["warnings"])
...
class TestKMeansValidator:
    ...
    def test_initialization(self):
        ...
        assert isinstance(validator.distribution_validator, CentroidDistributionValidator)
    ...
    async def test_validate_iteration_skipped(self):
        ...
        # First call should validate (iteration 1 % 2 != 0, but we start at 0)
        result = await validator.validate_iteration(losses, centroids_list)
        assert result["status"] == "completed"
        
        # Second call should be skipped (iteration 2 % 2 != 0)
        result = await validator.validate_iteration(losses, centroids_list)
    ...
    async def test_validation_with_problematic_losses(self):
        ...
        for result in later_results:
            if result["status"] == "completed" and result.get("warnings"):
                loss_warnings = [w for w in result["warnings"] if "Loss increased monotonically" in w]
                if loss_warnings:
                    warnings_found = True
                    break
