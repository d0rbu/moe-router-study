...
def get_router_weights(
    ...
    model = StandardizedTransformer(model_config.hf_name, device_map="cpu", revision=str(checkpoint))
    ...
    with th.no_grad():
        ...
        for layer_idx in tqdm(router_layers, desc="Getting router weights", total=len(router_layers), leave=False):
