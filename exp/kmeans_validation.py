...
class ValidationConfig:
    ...
    monotonicity_threshold: float = 0.01  # Minimum relative increase to consider significant
    
    # Centroid distribution validation
    validation_set_ratio: float = 0.1  # Fraction of data to use for validation
    min_points_per_centroid: int = 5  # Minimum points that should be assigned to each centroid
    max_empty_centroids_ratio: float = 0.1  # Maximum fraction of centroids that can be empty
    distribution_balance_threshold: float = 0.05  # Threshold for detecting severely imbalanced assignments
    ...
    def validate(self) -> dict[str, Any]:
        ...
        for k_idx in range(num_k_values):
            ...
            metrics[f"k_{k_idx}_relative_change"] = (
                ...
                if len(k_losses) >= 2 and k_losses[0] != 0 else 0.0
            )
...
class CentroidDistributionValidator:
    ...
    async def _validate_single_k(self, centroids: th.Tensor, k_idx: int) -> dict[str, Any]:
        ...
        distances = th.cdist(
            self.validation_data.to(th.float32), 
            centroids.to(th.float32), 
            p=1
        )
        ...
        under_populated = (assignment_counts < self.config.min_points_per_centroid).sum().item()
        ...
        if assignment_counts.numel() > 0:
            ...
            cv = std_assignments / mean_assignments if mean_assignments > 0 else float('inf')
...
class KMeansValidator:
    ...
    def should_validate(self) -> bool:
        ...
        return (
            ...
            self.iteration_count % self.config.validation_frequency == 0
        )
    
    async def validate_iteration(
        self, 
        losses: th.Tensor, 
        centroids_list: list[th.Tensor]
    ) -> dict[str, Any]:
        """
        ...
        distribution_results = await self.distribution_validator.validate(centroids_list)
        
        # Combine results
        all_warnings = loss_results.get("warnings", []) + distribution_results.get("warnings", [])
        all_metrics = {**loss_results.get("metrics", {}), **distribution_results.get("metrics", {})}
        
        # Log summary if there are warnings
        if all_warnings:
            logger.warning(f"Validation iteration {self.iteration_count}: {len(all_warnings)} warnings detected")
            for warning in all_warnings:
                logger.warning(f"  - {warning}")
        else:
            logger.debug(f"Validation iteration {self.iteration_count}: No issues detected")
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "state": {
                ...
                "has_validation_data": self.distribution_validator.validation_data is not None,
            }
        }
