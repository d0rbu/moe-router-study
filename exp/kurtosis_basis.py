...
def kurtosis_basis(
    ...
    # Dictionary to accumulate kurtosis values for each basis type
    # Format: basis_type -> list of kurtosis values across all elements
    kurtosis_by_basis: dict[str, list[th.Tensor]] = defaultdict(list)
    ...
    with th.no_grad():
        for batch in tqdm(activation_iterator, desc="Processing batches"):
            ...
            for layer_idx in tqdm(
                ...
                if layer_idx in router_layers:
                    ...
                    for expert_idx in range(num_experts):
                        ...
                        # Get weights
                        up_w = cast("Tensor", expert.up_proj.weight).detach()  # (mlp_dim, hidden_dim)
                        gate_w = cast("Tensor", expert.gate_proj.weight).detach()  # (mlp_dim, hidden_dim)
                        
                        # Project: (batch, mlp_dim) = (batch, hidden_dim) @ (hidden_dim, mlp_dim)
                        up_proj = pre_mlp_residuals @ up_w.T.to(pre_mlp_residuals.device)
                        gate_proj = pre_mlp_residuals @ gate_w.T.to(pre_mlp_residuals.device)
                    ...
                    layerwise_kurtosis[f"layer_{layer_idx}_gate_proj"][layer_idx].append(
                        gate_kurtosis.cpu()
                    )
                else:
                    # Dense MLP layer
                    up_w = cast("Tensor", model.mlps[layer_idx].up_proj.weight).detach()
                    gate_w = cast("Tensor", model.mlps[layer_idx].gate_proj.weight).detach()
            ...
            # 3. MLP down-projection neurons
            for layer_idx in tqdm(
                range(num_layers), desc="MLP down-proj", leave=False
            ):
                ...
                if layer_idx in router_layers:
                    ...
                    for expert_idx in range(num_experts):
                        ...
                        down_w = cast("Tensor", expert.down_proj.weight).detach()  # (hidden_dim, mlp_dim)
                        
                        # Project: (batch, mlp_dim) = (batch, hidden_dim) @ (hidden_dim, mlp_dim)
                        # We want to see what neurons the residual reads FROM
                        down_proj = post_mlp_residuals @ down_w.to(post_mlp_residuals.device)
                    ...
                    layerwise_kurtosis[f"layer_{layer_idx}_down_proj"][layer_idx].append(
                        down_kurtosis.cpu()
                    )
                else:
                    # Dense MLP layer
                    down_w = cast("Tensor", model.mlps[layer_idx].down_proj.weight).detach()
            ...
            for layer_idx in tqdm(
                router_layers, desc="Expert routers", leave=False
            ):
                # Get post-attention residuals for routing
                pre_mlp_residuals = attn_outputs[:, layer_idx, :]  # (batch, hidden_dim)
                
                # Get router weights
                router_weight = cast("Tensor", model.routers[layer_idx].weight).detach()  # (num_experts, hidden_dim)
                
                # Compute router logits: (batch, num_experts) = (batch, hidden_dim) @ (hidden_dim, num_experts)
                router_logits = pre_mlp_residuals @ router_weight.T.to(pre_mlp_residuals.device)
    ...
    for basis_name, layer_dict in layerwise_kurtosis.items():
        for layer_idx, kurtosis_list in layer_dict.items():
