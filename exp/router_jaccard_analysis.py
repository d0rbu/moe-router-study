...
def build_expert_activation_sets(
    router_logits: th.Tensor, top_k: int, token_offset: int = 0
...
def analyze_expert_coactivation(
    ...
    print(
        f"Analyzing {len(active_experts)} experts with >= {min_activations} activations"
    )
    ...
    return {
        ...
        "within_layer_distance_stats": {
            "mean": float(np.mean(within_layer_distances))
            if len(within_layer_distances) > 0
            else 0.0,
            "std": float(np.std(within_layer_distances))
            if len(within_layer_distances) > 0
            else 0.0,
            "min": float(np.min(within_layer_distances))
            if len(within_layer_distances) > 0
            else 0.0,
            "max": float(np.max(within_layer_distances))
            if len(within_layer_distances) > 0
            else 0.0,
            "median": float(np.median(within_layer_distances))
            if len(within_layer_distances) > 0
            else 0.0,
        },
        "within_layer_coefficient_stats": {
            "mean": float(np.mean(within_layer_coefficients))
            if len(within_layer_coefficients) > 0
            else 0.0,
            "std": float(np.std(within_layer_coefficients))
            if len(within_layer_coefficients) > 0
            else 0.0,
            "min": float(np.min(within_layer_coefficients))
            if len(within_layer_coefficients) > 0
            else 0.0,
            "max": float(np.max(within_layer_coefficients))
            if len(within_layer_coefficients) > 0
            else 0.0,
            "median": float(np.median(within_layer_coefficients))
            if len(within_layer_coefficients) > 0
            else 0.0,
        },
        # Cross-layer statistics
        "cross_layer_distance_stats": {
            "mean": float(np.mean(cross_layer_distances))
            if len(cross_layer_distances) > 0
            else 0.0,
            "std": float(np.std(cross_layer_distances))
            if len(cross_layer_distances) > 0
            else 0.0,
            "min": float(np.min(cross_layer_distances))
            if len(cross_layer_distances) > 0
            else 0.0,
            "max": float(np.max(cross_layer_distances))
            if len(cross_layer_distances) > 0
            else 0.0,
            "median": float(np.median(cross_layer_distances))
            if len(cross_layer_distances) > 0
            else 0.0,
        },
        "cross_layer_coefficient_stats": {
            "mean": float(np.mean(cross_layer_coefficients))
            if len(cross_layer_coefficients) > 0
            else 0.0,
            "std": float(np.std(cross_layer_coefficients))
            if len(cross_layer_coefficients) > 0
            else 0.0,
            "min": float(np.min(cross_layer_coefficients))
            if len(cross_layer_coefficients) > 0
            else 0.0,
            "max": float(np.max(cross_layer_coefficients))
            if len(cross_layer_coefficients) > 0
            else 0.0,
            "median": float(np.median(cross_layer_coefficients))
            if len(cross_layer_coefficients) > 0
            else 0.0,
        },
...
def analyze_router_jaccard(
    ...
    """
    print(
        f"Analyzing router coactivation with Jaccard distance for experiment: {experiment_name}"
    )
    ...
    print(
        f"Active experts (>= {min_activations} activations): {results['active_experts']}"
    )
    ...
    if results["within_layer_pairs"] > 0:
        ...
        print("\nWithin-layer Coactivation:")
        print(
            f"  Jaccard Distance: {within_dist['mean']:.4f} ± {within_dist['std']:.4f}"
        )
        print(
            f"  Jaccard Coefficient: {within_coeff['mean']:.4f} ± {within_coeff['std']:.4f}"
        )
    ...
    if results["cross_layer_pairs"] > 0:
        ...
        print(
            f"  Jaccard Coefficient: {cross_coeff['mean']:.4f} ± {cross_coeff['std']:.4f}"
        )

    # Most similar expert pairs
    print("\nMost Similar Expert Pairs (lowest Jaccard distance):")
    for i, ((expert_a, expert_b), distance, coefficient) in enumerate(
        results["most_similar_pairs"], 1
    ):
        layer_a, expert_a_idx = expert_a
        layer_b, expert_b_idx = expert_b
        layer_type = "within-layer" if layer_a == layer_b else "cross-layer"
        print(
            f"  {i:2d}. Layer {layer_a} Expert {expert_a_idx} ↔ Layer {layer_b} Expert {expert_b_idx}"
        )
        print(
            f"      Distance: {distance:.4f}, Coefficient: {coefficient:.4f} ({layer_type})"
        )

    # Most dissimilar expert pairs
    print("\nMost Dissimilar Expert Pairs (highest Jaccard distance):")
    for i, ((expert_a, expert_b), distance, coefficient) in enumerate(
        results["most_dissimilar_pairs"], 1
    ):
        layer_a, expert_a_idx = expert_a
        layer_b, expert_b_idx = expert_b
        layer_type = "within-layer" if layer_a == layer_b else "cross-layer"
    ...
    if results["within_layer_pairs"] > 0 and results["cross_layer_pairs"] > 0:
        ...
        if within_mean > cross_mean * 1.5:
            print(
                f"- Within-layer coactivation ({within_mean:.4f}) is much higher than cross-layer ({cross_mean:.4f})"
            )
        elif within_mean > cross_mean * 1.1:
            print(
                f"- Within-layer coactivation ({within_mean:.4f}) is higher than cross-layer ({cross_mean:.4f})"
            )
        elif cross_mean > within_mean * 1.1:
            print(
                f"- Cross-layer coactivation ({cross_mean:.4f}) is higher than within-layer ({within_mean:.4f})"
            )
        else:
            print(
                f"- Within-layer and cross-layer coactivation are similar ({within_mean:.4f} vs {cross_mean:.4f})"
            )
